...
A task scheduler interface is required if you intend on using `observe` (temporarily, will be changed in the future), animations (modules such as `confusion/motion`, `confusion/motion_springs`, `confusion/motion_tweens` or `confusion/chrono` are dependant on an external task scheduler) or anything within the `confusion/async` module.
